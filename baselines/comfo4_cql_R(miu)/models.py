    ...
    def critic_train_step(self,
        ...
        def loss_fn(params: FrozenDict):
            ...
            for _ in range(self.num_random):
                sub_key1, sub_key2 = jax.random.split(sub_key1, 2)
                perturbation = jax.random.uniform(
                    sub_key2, jnp.shape(batch.observations), minval=-self.noise_scale, maxval=self.noise_scale)
                ...
                diff = ood_noise_q - id_noise_q
                avg_dif += diff.mean()
            avg_dif /= self.num_random
            conservative_loss = self.conservative_weight*avg_dif
            # print("****************")
            ...
            # Repeat observations
            repeat_observations = jnp.repeat(jnp.expand_dims(
                batch.observations, axis=0), repeats=self.num_random, axis=0)

            # CQL loss
            sub_key1, sub_key2 = jax.random.split(sub_key1, 2)
            cql_random_actions = jax.random.uniform(
                sub_key1, shape=(self.num_random, self.act_dim), minval=-self.max_action, maxval=self.max_action)
            
            cql_random_q1, cql_random_q2 = self.critic.apply(
                {"params": params}, repeat_observations, cql_random_actions)
            ...
            cql_diff1 = jnp.clip(
                ood_q1 - q1, self.cql_clip_diff_min, self.cql_clip_diff_max)
            cql_diff2 = jnp.clip(
                ood_q2 - q2, self.cql_clip_diff_min, self.cql_clip_diff_max)
            cql_loss1 =  cql_diff1 * self.min_q_weight
            cql_loss2 =  cql_diff2 * self.min_q_weight
            R_miu =  cql_loss1 + cql_loss2

            return avg_critic_loss+conservative_loss+R_miu, {
                "critic_loss": avg_critic_loss,
                "max_critic_loss": critic_loss.max(),
                "min_critic_loss": critic_loss.min(),
                "avg_conservative_loss": avg_dif,
