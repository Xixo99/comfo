...
class COMFO3Agent:
    ...
    def value_train_step(self,
        ...
        _, sampled_actions, _ = self.actor.apply(
            {"params": actor_state.params}, self.rng, batch.observations)

        q1, q2 = self.critic.apply(
            {"params": critic_target_params}, batch.observations, sampled_actions)
        q = jnp.minimum(q1, q2)
    ...
    def train_step(self,
        ...
        value_info, new_value_state = self.value_train_step(
            batch, value_state, actor_state, critic_target_params)
