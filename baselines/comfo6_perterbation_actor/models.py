...
class COMFO6Agent:
    def __init__(self,
        ...
        self.actor = COMFO_Actor(act_dim=act_dim,
    ...
    def actor_alpha_train_step(self,
        ...
        def loss_fn(params, alpha_params):
            ...
            for _ in range(self.num_random):
                ...
                mle_prob = self.actor.apply({"params": params}, batch.observations+perturbation, batch.actions,
            ...
            loss = (actor_loss_mean + alpha_loss_mean)/self.num_random
            return loss, {
    ...
    def save(self, fname: str, cnt: int):
        checkpoints.save_checkpoint(
            fname, self.actor_state, cnt, prefix="actor_", keep=5, overwrite=True)
        checkpoints.save_checkpoint(
            fname, self.critic_state, cnt, prefix="critic_", keep=5, overwrite=True)
        checkpoints.save_checkpoint(
            fname, self.value_state, cnt, prefix="value_", keep=5, overwrite=True)
