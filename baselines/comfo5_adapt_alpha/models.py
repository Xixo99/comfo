...
class COMFO3Agent:
    def __init__(self,
        ...
        self.rng, conservative_key = jax.random.split(self.rng, 2)
        self.log_conservative_weight = Scalar(0.0)
        self.conservative_weight_state = train_state.TrainState.create(
            apply_fn=None,
            params=self.log_conservative_weight.init(conservative_key)[
                "params"],
            tx=optax.adam(lr)
        )
    ...
    def critic_train_step(self,
                          ...
                          conservative_weight_state: train_state.TrainState,
        ...
        def loss_fn(params: FrozenDict, conservative_weight_params):
            q1, q2 = self.critic.apply(
            ...
            log_conservative_weight = self.log_conservative_weight.apply(
                {"params": conservative_weight_params})
            conservative_weight_loss = -log_conservative_weight * \
                jax.lax.stop_gradient(avg_conservative_loss-2)

            conservative_weight = jnp.exp(log_conservative_weight)
            conservative_weight = jax.lax.stop_gradient(conservative_weight)
            conservative_loss = conservative_weight*avg_conservative_loss

            return avg_critic_loss+conservative_loss+conservative_weight_loss, {
        ...
        (_, critic_info), grads = jax.value_and_grad(loss_fn, argnums=(0, 1),
                                                     has_aux=True)(critic_state.params, conservative_weight_state.params)
        critic_grads, conservative_weight_grads = grads

        critic_state = critic_state.apply_gradients(grads=critic_grads)
        conservative_weight_state = conservative_weight_state.apply_gradients(
            grads=conservative_weight_grads)

        return critic_info, critic_state, conservative_weight_state
    ...
    def train_step(self,
        ...
        critic_info, new_critic_state, new_conservative_weight_state = self.critic_train_step(
            batch, critic_state, conservative_weight_state, new_value_state.params)
        ...
        return new_alpha_state, new_actor_state, new_value_state, new_critic_state, new_conservative_weight_state, new_critic_target_params, {
            **actor_info, **value_info, **critic_info}

    def update(self, batch: Batch):
        self.rng, update_rng = jax.random.split(self.rng, 2)
        (self.alpha_state, self.actor_state, self.value_state, self.critic_state, self.conservative_weight_state,
         self.critic_target_params, log_info) = self.train_step(batch,
                                                                ...
                                                                self.value_state,
                                                                self.critic_state,
                                                                self.conservative_weight_state,
