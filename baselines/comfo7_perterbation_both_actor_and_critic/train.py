...
from models import COMFO7Agent
...
def train_and_evaluate(configs: ml_collections.ConfigDict):
    ...
    agent = COMFO7Agent(obs_dim=obs_dim,
                        act_dim=act_dim,
    ...
    for t in trange(1, configs.max_timesteps+1):
        ...
        if (t > int(9.5e5) and (t % configs.eval_freq == 0)) or (t <= int(9.5e5) and t % (2*configs.eval_freq) == 0):
            ...
            logger.info(
                ...
                f"\tavg_conservative_loss: {log_info['avg_conservative_loss']: .3f}, conservative_weight: {log_info['conservative_weight']: .3f}\n"
                f"\tavg_smooth_loss: {log_info['avg_smooth_loss']: .3f}\n"
            )
